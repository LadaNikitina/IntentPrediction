...
class FastGTNs(nn.Module):
    def __init__(self, num_edge_type, w_in, num_class, num_nodes, args=None):
        ...
        linear_weights = np.zeros(self.args.top_k)
        linear_weights[...] = 1 / self.args.top_k
    ...
    def forward(self, A, X, num_nodes=None, args=None, n_id=None, node_labels=None, epoch=None):
        if num_nodes == None:
        ...
        h = torch.reshape(H_, (len(H_) // self.args.top_k, self.args.top_k, self.args.node_dim))        
        linear_weights_1dim = torch.reshape(self.linear_weights.weight, (self.args.top_k, ))
...
class FastGTN(nn.Module):
    def __init__(self, num_edge_type, w_in, num_class, num_nodes, args=None, pre_trained=None):
        ...
        for i in range(self.num_layers+1):
            feat_trans_layers.append(nn.Sequential(nn.Linear(self.w_out, 256),
                                        nn.ReLU(),
                                        nn.Linear(256, 128)))
...
class FastGTLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, num_nodes, first=True, args=None, pre_trained=None):
        ...
        self.args = args
        self.feat_transfrom = nn.Sequential(nn.Linear(args.w_in, 256),
                                        nn.ReLU(),
                                        nn.Linear(256, 128))
