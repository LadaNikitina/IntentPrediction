...
def get_metric_intent(data_df, num_clusters_cur):
    '''
        major intent metric
    '''
    most_common_intents = []
    ...
    for i in range(num_clusters_cur):
        intents_i = data_df[data_df['cluster'] == i]['intent']
        intents = []

        for intent in intents_i:
            intents += intent

        if (len(intents) != 0):
            counter = Counter(intents)
            most_common_intent = counter.most_common(1)[0][0]
            most_common_intents.append(most_common_intent)
            num = 0

            for intent in intents_i:
                if most_common_intent in intent:
                    num += 1
            part_intents.append(num / len(intents_i))

    return np.array(part_intents).mean()

class Clusters:
    ''' 
        the class that forms clusters
    '''
    def __init__(self, first_num_clusters, second_num_clusters, path):
    ...
    def train_test_valid(self):
        ...
        validation_df = get_pd_utterances_speaker(self.validation_dataset)
        ...
        self.user_valid_df = validation_df[validation_df["speaker"] == 0].reset_index(drop=True)
        ...
        self.system_valid_df = validation_df[validation_df["speaker"] == 1].reset_index(drop=True)
        ...
        self.user_valid_index = validation_df[validation_df["speaker"] == 0].index \
        ...
        self.system_valid_index = validation_df[validation_df["speaker"] == 1].index \
                                  + len(train_df) + len(test_df)
    ...
    def get_embeddings(self):
        ...
        '''
        embeddings = np.array(pd.read_csv(self.path, index_col=0))
    ...
    def one_stage_clustering(self):
        ...
        '''
        print(f"Intent metric, {self.first_num_clusters} one-stage clusters, \
                user: {get_metric_intent(self.train_user_df_first_stage, self.first_num_clusters)}")
        print(f"Intent metric, {self.first_num_clusters} one-stage clusters, \
                system: {get_metric_intent(self.train_system_df_first_stage, self.first_num_clusters)}")
    ...
    def two_stage_clustering(self):
        ...
        '''
        print(f"Intent metric, {self.second_num_clusters} two-stage clusters, \
                user: {get_metric_intent(self.train_user_df_sec_stage, self.second_num_clusters)}")
        print(f"Intent metric, {self.second_num_clusters} two-stage clusters, \
                system: {get_metric_intent(self.train_system_df_sec_stage, self.second_num_clusters)}")
